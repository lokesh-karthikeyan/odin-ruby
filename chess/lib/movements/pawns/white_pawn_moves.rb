...
# Computes the White Pawn's possible legal moves.
class WhitePawnMoves
  ...
  def advance_two_steps?(first_step) = first_step.any? && spot.first.eql?(6)

  def move_forward(moves = [], double: false)
    one_step = Routes::WHITE_PAWN_SINGLE_ADVANCE
    two_step = Routes::WHITE_PAWN_DOUBLE_ADVANCE
  ...
  def capture_moves(moves = [])
    left_diagonal_location = compute_position(Routes::WHITE_PAWN_LEFT_CAPTURE, spot)
    right_diagonal_location = compute_position(Routes::WHITE_PAWN_RIGHT_CAPTURE, spot)
